...
def train(_):
    ...
    X_train = (X - X.mean(axis=0)) / (X.std(axis=0) + 1e-4)
    X_train = X_train.reshape(mnist.train.images.shape[0], 28, 28, 1)

    X = mnist.test.images
    X_test = (X - X.mean(axis=0)) / (X.std(axis=0) + 1e-4)
    ...
    def feed_dict(train):
        if train:
            ...
            xs, ys = X_test, mnist.test.labels
            k = 1.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str,
                        default=os.path.join(os.getenv('TEST_TMPDIR', '/tmp'), 'tf/mnist/augm_bn_norm'),
